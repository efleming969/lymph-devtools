...
const zipModule = function (module_name) {
    ...
    return new Promise(function (resolve, reject) {
        const output = FS.createWriteStream(Path.join(build_dir, `${module_name}.zip`));
...
const updateFunction = function (module_name) {
    const lambda = new aws_sdk_1.Lambda({ region: "us-east-1" });
...
module_files.forEach(function (module_file) {
    ...
    child_process_1.execFile(Path.join(npm_bin, "tsc"), tsc_options, function (err) {
